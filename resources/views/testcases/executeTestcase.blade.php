            ...
            if (service) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        projectDropdown.empty().append('<option value="" disabled selected>Select Project</option>');

                        if ($.isEmptyObject(data)) {
                            projectDropdown.append('<option value="" disabled>No Projects Available</option>');
                        } else {
                ...
                $('#project_id').empty().append('<option value="" disabled selected>Select Project</option>');
            }
        });

        // Fetch test cases when a project is selected
        $('#project_id').change(function() {
            let projectId = $(this).val();
            if (projectId) {
                $.ajax({
                    url: "{{ route('testcases.getTestCaseData') }}",
                    type: "POST",
                    data: {
                    ...
                    success: function(testCases) {
                        let tableBody = $('#testcases-table tbody');
                        tableBody.empty();

                        if (testCases.length === 0) {
                            tableBody.append('<tr><td colspan="7" class="text-center">No test cases found.</td></tr>');
                        } else {
                            $.each(testCases, function(index, testCase) {
                                tableBody.append(
                                    `<tr>
                                        ...
                                        <td>${testCase.test_environment}</td>
                            ...
                            // Fill form with first test case data
                            let firstTestCase = testCases[0];
                            $('#tester').val(firstTestCase.tester);
                            $('#test_environment').val(firstTestCase.test_environment);
                            $('#test_case_no').val(firstTestCase.test_case_no);
                            $('#test_title').val(firstTestCase.test_title);
                            $('#test_step').val(firstTestCase.test_step);
                            $('#category_id').val(firstTestCase.category_id);
                            $('#date_of_input').val(firstTestCase.date_of_input);
                            $('#priority').val(firstTestCase.priority);
                        }
                    },
                    error: function(xhr) {
                        console.error(xhr.responseJSON);
